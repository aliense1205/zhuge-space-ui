...
const GlobalStyles = () => (
    <style>
      ...
      /* 强制 Tailwind 识别自定义颜色，避免 IFrame 样式丢失 */
      .text-red-400, .bg-red-500, .border-red-500, .shadow-red-500, .bg-orange-400, .text-orange-400, .bg-cyan-500, .text-cyan-400, .text-green-400, .bg-green-500, .text-blue-400 {}
    `}
    </style>
);

// ==========================================
// B. Components: Icons (保持不变)
// ==========================================
...
const Icons = {
    ...
    Briefcase: (p: IconProps) => <IconWrapper {...p}><rect x="2" y="7" width="20" height="14" rx="2" ry="2" /><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16" /></IconWrapper>,
    Map: (p: IconProps) => <IconWrapper {...p}><polygon points="1 6 1 22 8 18 16 22 23 18 23 2 16 6 8 2 1 6" /><line x1="8" y1="2" x2="8" y2="18" /><line x1="16" y1="6" x2="16" y2="22" /></IconWrapper>,
    Users: (p: IconProps) => <IconWrapper {...p}><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M23 21v-2a4 4 0 0 0-3-3.87" /><path d="M16 3.13a4 4 0 0 1 0 7.75" /></IconWrapper>,
    Wallet: (p: IconProps) => <IconWrapper {...p}><rect x="1" y="4" width="22" height="16" rx="2" ry="2" /><line x1="1" y1="10" x2="23" y2="10" /></IconWrapper>
};

// ==========================================
// C. Components: Shared Helpers (保持不变)
// ==========================================
...
// ==========================================
// 2. Scene: Cockpit (驾驶舱) - 核心修改区域
// ==========================================

const SceneCockpit = () => {
    // 🔔 HOOKS: 引入诸葛 Agent 核心 Hook
    ...
    return (
        <div className="relative w-full h-full bg-slate-950 overflow-hidden text-white group select-none">
            ...
            {/* 🟢 [核心修改]：替换右侧 IFrame 为原生 React Chat 组件 */}
            <div className="absolute right-0 top-0 h-full w-[30%] z-50 border-l border-slate-800 bg-slate-900/80 backdrop-blur-sm">
                <NativeChat
                    messages={messages}
                    loading={loading}
                    onSend={sendMessage}
            ...
            {/* 主背景层 (占据左侧 70%) */}
            <div className="absolute inset-0 z-0 overflow-hidden bg-slate-950" style={{ width: '70%' }}>
                
                {/* 🟢 [核心修改] 视觉拦截层：如果 visualState 不是 default，则显示 Agent 的图表 */}
                {(visualState.type && visualState.type !== 'default' && visualState.type !== 'none') ? (
                    <div className="absolute inset-0 z-[60] bg-slate-950 p-6 animate-in fade-in zoom-in duration-300">
                        <VisualStage visualState={visualState} />
                        
                        {/* 简易关闭/重置按钮 */}
                        <button 
                             onClick={() => window.location.reload()}
                             className="absolute top-4 right-4 flex items-center gap-1 bg-slate-800/80 hover:bg-slate-700 text-slate-300 hover:text-white px-3 py-1.5 rounded-full text-xs transition-all border border-slate-700 backdrop-blur-md"
                        >
                            <Icons.Activity className="w-3 h-3" /> 返回态势图
                        </button>
                    </div>
                ) : (
                    /* 如果没有特殊视觉指令，显示原地图和浮动 UI */
                    <>
                        ...
                        <div className="absolute inset-0 z-10 p-6 flex flex-col justify-between">
                            ...
                            <div className="flex justify-between items-start">
                                ...
                                <div className="animate-in slide-in-from-top fade-in duration-700">
                                    <div className="flex items-center gap-2 mb-1">
                                        ...
                                        <span className="px-1.5 py-0.5 rounded bg-blue-900/50 border border-blue-500/30 text-[10px] text-blue-300 ml-2">视角: 政府A面</span>
                                    </div>
                            ...
                            <div className="absolute top-[30%] left-[20%] flex items-center gap-2 z-50">
                                ...
                                <div className="bg-black/70 backdrop-blur px-3 py-1 border-l-2 border-red-500 text-xs">
                                    ...
                                    <div className="text-[10px] text-gray-300">F3层物理隔离 · 严密</div>
                                </div>
                            </div>
                            ...
                            <div className="absolute top-[35%] right-[10%] w-[260px] bg-black/80 backdrop-blur-md p-3 rounded-lg border border-emerald-500/50 shadow-lg z-50">
                                <div className="flex items-center gap-2 mb-3 justify-between">
                                    ...
                                    {/* 亮点：AI练习生计划 */}
                                    <span className="px-1.5 py-0.5 rounded bg-emerald-900/80 border border-emerald-400/50 text-[9px] text-emerald-300 animate-pulse">AI练习生</span>
                                </div>
                                <div className="space-y-2">
                                    {/* 🧠 核心升级：体现管理端的“规划与选址”能力 */}
                                    <KVPairGrow k="多智能体协同" v="运行中" vColor="text-green-400 animate-pulse" />
                                    <KVPairGrow k="产业规划推演" v="方案 V2.1 生成" vColor="text-cyan-300" />
                                    <KVPairGrow k="空间选址匹配" v="3 处地块待审" vColor="text-orange-400" />
...
// ==========================================
// 3. Scene: Smart Apps (智能应用) (保持不变)
// ==========================================
const SceneApps = () => {
    // 🔔 HOOKS: 模拟 Agent 状态获取 (已重构)
    const [recommendedCount, setRecommendedCount] = useState(30);
    const [investmentStatus, setInvestmentStatus] = useState<'analyzing' | 'matching'>('analyzing');
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            ...
            // 模拟状态切换
            setInvestmentStatus(prev => prev === 'analyzing' ? 'matching' : 'analyzing');
        }, 2000);
    ...
    const INVESTMENT_STATUS_MAP = {
        analyzing: { text: 'text-orange-400', indicator: 'bg-orange-500 animate-pulse', label: '企业需求分析' },
        matching: { text: 'text-green-400', indicator: 'bg-green-500', label: '政策匹配中' }
    };
    const currentInvestment = INVESTMENT_STATUS_MAP[investmentStatus];
        ...
        <div className="w-full h-full bg-gray-950 font-sans relative overflow-y-auto text-white">
            ...
            <div className="relative w-full h-[65vh] bg-slate-900">
                ...
                <div className="absolute top-0 w-full p-6 flex justify-between items-center text-white z-10">
                    ...
                    <div className="flex gap-4 items-center">
                        <span className="hidden md:block px-3 py-1 rounded-full bg-indigo-900/50 border border-indigo-500/30 text-xs text-indigo-300">视角: 企业B面</span>
            ...
            <div className="relative -mt-10 z-50 px-6 md:px-12 pb-24 grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-slate-900 p-6 rounded-2xl shadow-xl border border-slate-700 group hover:border-indigo-600 transition-colors cursor-pointer">
                    ...
                    <p className="text-slate-400 text-sm mb-4">企业专属智能服务 (B-Side Services)</p>

                    <div className="space-y-3">
                        {/* 1. 招商/入驻咨询 (服务入口) */}
                        <div className="flex items-center justify-between p-2 rounded-lg bg-gray-900/40 hover:bg-indigo-900/40 transition-colors border-l-2 border-orange-500/50">
                            <div className="flex items-center gap-3">
                                <div className="w-8 h-8 rounded-full bg-orange-900/40 flex items-center justify-center text-xs font-bold text-orange-400"><Icons.Briefcase className="w-4 h-4" /></div>
                                <div><p className="text-sm font-semibold text-white leading-none">企业入驻/招商咨询</p><p className={`text-[10px] ${currentInvestment.text} transition-colors`}>{currentInvestment.label}</p></div>
                            </div>
                            <div className={`text-xs ${currentInvestment.text} flex items-center gap-1`}><span className={`w-2 h-2 rounded-full ${currentInvestment.indicator}`}></span></div>
                        </div>

                        {/* 2. 政策申报 (服务支持) */}
                        <div className="flex items-center justify-between p-2 rounded-lg bg-gray-900/40 hover:bg-indigo-900/40 transition-colors border-l-2 border-indigo-500/50">
                            <div className="flex items-center gap-3">
                                ...
                                <div><p className="text-sm font-semibold text-white leading-none">政策申报助手</p><p className="text-[10px] text-slate-400">匹配中：[高新企业补贴]</p></div>
                            </div>
                            <div className="text-xs text-green-400 flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-500 animate-ping-slow"></span><span>服务中</span></div>
                        </div>

                        {/* 3. 人才/招聘服务 (服务支持) */}
                        <div className="flex items-center justify-between p-2 rounded-lg bg-gray-900/40 hover:bg-indigo-900/40 transition-colors border-l-2 border-green-500/50">
                            <div className="flex items-center gap-3">
                                <div className="w-8 h-8 rounded-full bg-green-900/40 flex items-center justify-center text-xs font-bold text-green-400"><Icons.Users className="w-4 h-4" /></div>
                                <div><p className="text-sm font-semibold text-white leading-none">人才招聘助手</p><p className="text-[10px] text-slate-400 transition-colors">今日匹配简历 <span className="text-yellow-400 font-bold">{recommendedCount}</span> 份</p></div>
                            </div>
                        ...
                        {/* 4. 企业金融服务 (新增：替换掉规划，符合B端需求) */}
                        <div className="flex items-center justify-between p-2 rounded-lg bg-gray-900/40 hover:bg-indigo-900/40 transition-colors border-l-2 border-blue-500/50">
                            <div className="flex items-center gap-3">
                                <div className="w-8 h-8 rounded-full bg-blue-900/40 flex items-center justify-center text-xs font-bold text-blue-400"><Icons.Wallet className="w-4 h-4" /></div>
                                <div><p className="text-sm font-semibold text-white leading-none">企业金融服务</p><p className="text-[10px] text-slate-400">园区贷/融资对接</p></div>
                            </div>
                            <div className="text-xs text-blue-400 flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-blue-500"></span><span>在线</span></div>
                        </div>
    ...
    <div className="w-full h-full bg-[radial-gradient(circle_at_center,_#000000_20%,_#064e3b_100%)] flex flex-col items-center justify-center relative overflow-hidden">
        ...
        <div className="relative z-20 text-center">
            ...
            <div className="flex flex-col gap-1 items-center mb-6">
                ...
                <p className="text-white/60 text-xs">F3层物理隔离 · 数据可用不可见</p>
            </div>
